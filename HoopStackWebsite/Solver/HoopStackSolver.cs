...
/*        public void AddLevel(string productId, int rating)
        ...
        public static bool solveLevel(Level level) //algorithm to solve the puzzle 
        {
            ...
            int numSolutions = solutions.moveLists.Count;

            if (numSolutions != 0) //if there a prev solution, follow steps until an error of INF or ERR
            {
                List<Move> prevSol = solutions.moveLists[numSolutions - 1]; //steps of prev sol
            ...
            { //if incorrect solution was found, try again with recursion
                Console.WriteLine("\n\n\t INCORRECT \n\n");
                solutions.moveLists.Add(sol); //add prev incorrect solution to solutions
                //reset the levels stacks to the original copied stacks, update solutions and wrongmoves
                level.Stacks = stacksCopy;
                level.Solutions = solutions;
                level.WrongMoves = wrongMoves;
                level.Error = solutions.moveLists[^1][^1].color;     //error is the 'Color' of the last move in the last solution
                return solveLevel(level);
            }
        }
        public static bool solve(List<List<string>> stacks, Moves solutions, List<Move> wrongMoves) //algorithm to solve the puzzle 
        {
            List<List<string>> stacksCopy = new List<List<string>>(); //copy of stacks
            foreach (List<string> s in stacks)
            {
                        ...
                        { //all stacks 
                            ...
                            { //if stack colors match and stack is not minstack
                                ...
                                { //for all consecutive matching colors 
                                    ...
                                    if (stacks[tempCount].Count != 0)
                                        tempTos = stacks[tempCount][stacks[tempCount].Count - 1]; //update temp tos after move
                                }
                            }
                            tempCount++;
                        }
            ...
            { //if incorrect solution was found, try again with recursion
                Console.WriteLine("\n\n\t INCORRECT \n\n");
                solutions.moveLists.Add(sol); //add prev incorrect solution to solutions
                return solve(stacksCopy, solutions, wrongMoves);
            }
        }
